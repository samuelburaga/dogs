(() => {
	"use strict";
	var e,
		i = {},
		d = {};
	function n(e) {
		var l = d[e];
		if (void 0 !== l) return l.exports;
		var r = (d[e] = { exports: {} });
		return i[e](r, r.exports, n), r.exports;
	}
	(n.m = i),
		(e = []),
		(n.O = (l, r, s, f) => {
			if (!r) {
				var c = 1 / 0;
				for (a = 0; a < e.length; a++) {
					for (var [r, s, f] = e[a], t = !0, o = 0; o < r.length; o++)
						(!1 & f || c >= f) && Object.keys(n.O).every((p) => n.O[p](r[o]))
							? r.splice(o--, 1)
							: ((t = !1), f < c && (c = f));
					if (t) {
						e.splice(a--, 1);
						var u = s();
						void 0 !== u && (l = u);
					}
				}
				return l;
			}
			f = f || 0;
			for (var a = e.length; a > 0 && e[a - 1][2] > f; a--) e[a] = e[a - 1];
			e[a] = [r, s, f];
		}),
		(n.o = (e, l) => Object.prototype.hasOwnProperty.call(e, l)),
		(() => {
			var e = { 666: 0 };
			n.O.j = (s) => 0 === e[s];
			var l = (s, f) => {
					var o,
						u,
						[a, c, t] = f,
						v = 0;
					if (a.some((h) => 0 !== e[h])) {
						for (o in c) n.o(c, o) && (n.m[o] = c[o]);
						if (t) var _ = t(n);
					}
					for (s && s(f); v < a.length; v++) n.o(e, (u = a[v])) && e[u] && e[u][0](), (e[u] = 0);
					return n.O(_);
				},
				r = (self.webpackChunkdogs = self.webpackChunkdogs || []);
			r.forEach(l.bind(null, 0)), (r.push = l.bind(null, r.push.bind(r)));
		})();
})();
